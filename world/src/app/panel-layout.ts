...
export class PanelLayoutManager implements AppModule {
  ...
  renderLayout(): void {
    this.ctx.container.innerHTML = `
      ...
      <div class="main-content no-right-sidebar${this.ctx.isDesktopApp ? ' desktop-grid' : ''}">
  ...
  private createPanels(): void {
    ...
    // Always create Pro add-panel block — show/hide reactively via auth state.
    ...
    if (import.meta.env.DEV) {
      ...
      const extra = [...created].filter(k => !configured.has(k) && k !== 'runtime-config' && !k.startsWith('cw-'));
