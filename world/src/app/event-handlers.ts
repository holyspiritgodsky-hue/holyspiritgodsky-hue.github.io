...
export class EventHandlerManager implements AppModule {
  ...
  private setupEventListeners(): void {
    ...
    if (this.ctx.isDesktopApp) {
      ...
      this.boundDesktopExternalLinkHandler = (e: MouseEvent) => {
        ...
        // Image URLs should bypass desktop bridge to avoid intrusive confirmation popups.
        const imagePath = url.pathname.toLowerCase();
        const isImageUrl = /\.(png|jpe?g|gif|webp|svg|avif|bmp|ico)$/.test(imagePath);
        if (isImageUrl) {
  ...
  setupMapResize(): void {
    ...
    if (mainContent?.classList.contains('no-right-sidebar')) {
      localStorage.removeItem('map-height');
