...
export class App {
  ...
  constructor(containerId: string) {
    ...
    // Panels that must survive variant switches: desktop config and user-created widgets.
    const isDynamicPanel = (k: string) => k === 'runtime-config' || k.startsWith('cw-');
    ...
    // Force-hide right-side AI panels across variants to simplify the main view.
    {
      const HIDDEN_RIGHT_PANEL_KEYS = new Set(['insights', 'strategic-posture', 'forecast']);
      let changed = false;
      for (const key of HIDDEN_RIGHT_PANEL_KEYS) {
        const current = panelSettings[key];
        if (current?.enabled) {
          panelSettings[key] = { ...current, enabled: false };
          changed = true;
        }
      }
      if (changed) {
